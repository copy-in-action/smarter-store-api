...
| 버전 | 날짜 | 작성자 | 변경 내용 |
|------|------|--------|----------|
| 1.0 | 2025-12-22 | BE | 초안 작성 - 일괄 점유 및 SSE 설계 |
| 1.1 | 2025-12-26 | Claude | ScheduleSeatStatus.seatGrade 필드 추가, 등급별 집계 최적화 |
...
## 6. v1.1 변경사항 (2025-12-26)

### 6.1 ScheduleSeatStatus에 seatGrade 필드 추가

좌석 점유 시 해당 좌석의 등급을 함께 저장하여, 잔여석 조회 시 JSON 파싱 없이 DB에서 직접 등급별 집계가 가능합니다.

```kotlin
class ScheduleSeatStatus(
    // ... 기존 필드
    val seatGrade: SeatGrade,  // 추가
    // ...
)
```
...
fun countByScheduleIdGroupBySeatGrade(scheduleId: Long): List<Array<Any>>
```
...
| 항목 | Before | After |
|------|--------|-------|
| 잔여석 조회 | seatingChart JSON 파싱 필요 | DB 집계 쿼리만 수행 |
| 복잡도 | O(n) - 좌석별 등급 파싱 | O(1) - GROUP BY 쿼리 |
