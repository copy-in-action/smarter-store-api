...
- **현재 로그**: 항상 `smarter-store.log` 파일에 기록됩니다.
- **날짜/용량 기반 롤링**: 날짜가 변경되거나 파일이 `10MB`를 초과하면, 날짜+순번이 붙은 아카이브 파일로 롤오버됩니다.
...
```
logs/
├── smarter-store.log            ← 현재 로그 (항상 여기에 기록)
├── smarter-store-2025-12-15.0.log  ← 롤오버된 아카이브
├── smarter-store-2025-12-15.1.log  ← 용량 초과 시 추가 아카이브
└── smarter-store-2025-12-14.0.log  ← 이전 날짜 로그
```
