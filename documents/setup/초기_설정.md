...
DDD의 핵심인 도메인 객체를 정의합니다. 예를 들어 `Performance`라는 도메인이 있다면 아래와 같이 `Performance.kt` 파일을 생성합니다.
...
class PerformanceController(
    ...
    @GetMapping("/{id}")
    fun getPerformanceById(@PathVariable id: Long): ResponseEntity<PerformanceResponse> {
...
애플리케이션이 성공적으로 실행되면, `http://localhost:8080/api/performances/{id}`와 같은 주소로 요청을 보내 API를 테스트할 수 있습니다.
