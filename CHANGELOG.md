...
## 2026년 2월 7일 (토)
*   **마이페이지 및 관리자용 예매 내역 조회 기능 구현 [CCS-158]:**
    *   **사용자용 내역/상세**: 사용자가 자신의 예매 목록(`GET /api/bookings/me`)과 공연/결제 정보가 포함된 상세 데이터(`GET /api/bookings/{id}`)를 조회할 수 있는 API를 구현했습니다.
    *   **관리자용 현황/상세**: 특정 회차의 전체 예매 목록(`GET /api/admin/bookings/schedules/{id}`) 및 특정 예매 건의 상세 내역 조회 API를 구축했습니다.
    *   **데이터 통합 제공**: 예매, 공연, 좌석 상세, 결제 승인 정보를 한눈에 확인할 수 있는 통합 DTO(`BookingDetailResponse`)를 적용했습니다.
    *   **가이드 신설**: 상세 명세를 담은 `예매_내역_조회_가이드.md`를 새로 작성했습니다.
    
*   **예매 취소 통합 프로세스 및 내역 조회 API 구현 [CCS-156]:**
    *   **예매 취소 및 결제 환불 통합**: 중복된 결제 취소 API를 제거하고 `DELETE /api/bookings/{id}` API로 취소 프로세스를 단일화했습니다. `BookingService`에서 환불 및 쿠폰 복구를 통합 제어합니다.
    *   **관리자 강제 취소 API**: `DELETE /api/admin/bookings/{bookingId}`를 통해 관리자가 소유권 검증 없이 예매를 취소할 수 있는 기능을 추가하고 Audit Log를 적용했습니다.
    *   **마이페이지 예매 내역 및 상세 조회**: 사용자가 본인의 확정/취소 내역을 목록(`GET /api/bookings/me`) 또는 단건 상세(`GET /api/bookings/{bookingId}`)로 확인할 수 있는 API를 구현했습니다.
    *   **관리자용 회차별 목록 및 상세 조회**: 특정 회차의 전체 목록(`GET /api/admin/bookings/schedules/{id}`) 및 특정 예매의 상세 데이터(`GET /api/admin/bookings/{id}`)를 조회하는 기능을 구축했습니다. (N+1 문제 해결)
    *   **통합 데이터 제공**: 예매 정보와 함께 공연 정보, 좌석 상세, 결제 완료 내역(PG 승인 정보 포함)을 한꺼번에 확인할 수 있도록 구조화했습니다.
