...
*   **Facade(Use Case) 계층 도입을 통한 아키텍처 고도화 [CCS-174]**:
    *   **결제 및 예매 오케스트레이션 분리**: `PaymentFacade`와 `BookingFacade`를 신설하여 여러 도메인(예매, 결제, 쿠폰) 간의 복잡한 조율 로직을 서비스 계층에서 분리했습니다.
    *   **단일 책임 원칙(SRP) 강화**: `PaymentService`와 `BookingService`가 외부 도메인 의존성 없이 자신의 도메인 규칙 및 데이터 관리에만 집중할 수 있도록 리팩토링했습니다.
    *   **이벤트 발행 책임 이관**: 외부 시스템(통계, 알림 등)과의 연동을 위한 도메인 이벤트 발행 책임을 Facade 계층으로 옮겨 도메인 서비스의 순수성을 확보했습니다.
*   **동시성 제어 및 데이터 무결성 강화**:
    *   **낙관적 락(Optimistic Lock) 도입**: `Payment` 엔티티에 JPA `@Version`을 적용하여 중복 결제 완료 승인 및 상태 변경 충돌을 원천 차단했습니다.
    *   **비관적 락(Pessimistic Lock) 적용**: `CouponService.useCoupons()` 시 쿠폰 조회 단계에서 `PESSIMISTIC_WRITE` 락을 사용하여 고부하 상황에서의 쿠폰 오남용 및 데이터 꼬임을 방지했습니다.
*   **JPA 성능 최적화 및 N+1 문제 해결**:
    *   **Fetch Join 전수 적용**: `PerformanceScheduleRepository`의 주요 쿼리에 `JOIN FETCH`를 적용하여 공연 정보 및 티켓 옵션 조회 시 발생하는 N+1 문제를 해결했습니다.
    *   **Querydsl DTO 프로젝션**: 공연 검색 및 자동완성 쿼리에서 엔티티 대신 DTO 프로젝션을 사용하여 불필요한 데이터 조회를 최소화하고 성능을 극대화했습니다.
*   **코드 품질 및 DX(Developer Experience) 개선**:
    *   **에러 코드 표준화**: 엔티티 내부의 하드코딩된 한글 메시지를 `ErrorCode` 체계로 전면 대체했습니다.
    *   **통합 테스트 구축**: 복합 필터링이 포함된 `PerformanceRepository`의 Querydsl 검색 로직에 대한 DB 통합 테스트를 추가했습니다.
    *   **기술 문서 보강**: `Facade_패턴과_유스케이스_계층_이해.md`를 실제 구현 사례 중심으로 개정하고, FE 개발자를 위한 `리팩토링_API_검증_가이드.md`를 신규 작성했습니다.
...
*   **공연 검색 및 자동완성 시스템 구축 [CCS-170]**:
    *   **통합 검색 API**: 제목, 카테고리(장르), 공연장 주소를 통합 검색하는 API(`GET /api/performances/search`)를 구현했습니다.
    *   **다중 선택 필터링**: 판매 상태(예정/중/종료), 장르, 지역 필터에 대해 다중 선택(`OR` 조건) 및 필터 간 교차 필터링(`AND` 조건)을 지원합니다.
    *   **17개 행정구역 매핑**: 대한민국 17개 광역자치단체를 정의하는 `Region` Enum을 신설하고, 공연장 주소에서 지역을 자동 추출하는 정교한 매핑 로직을 구축했습니다.
    *   **검색 자동완성**: 검색어 입력 시 최대 6개의 관련 공연을 실시간 추천하는 API(`GET /api/performances/search/autocomplete`)를 추가했습니다. 판매 종료된 공연은 노출되지 않도록 최적화했습니다.
    *   **Querydsl 5.1.0 도입**: 복합 동적 쿼리와 예매 수량 집계 기반 정렬(예매 많은 순)을 타입 안정적으로 처리하기 위해 Querydsl(Jakarta 버전)을 도입하고 `JPAQueryFactory` 설정을 완료했습니다.
    *   **성능 최적화**: 목록 조회 시 불필요한 찜(`isWishlisted`) 여부 확인 및 필터별 상세 집계 로직을 제거하여 검색 응답 속도를 극대화했습니다.
    *   **Swagger 및 DX 개선**: 검색 API의 파라미터별 Enum 명세와 설명을 보강하고, `GET /api/enums` API에 신규 Enum 3종을 추가하여 프론트엔드 연동 편의성을 높였습니다.
    *   **문서화**: 설계서, TODO 리스트, FE 개발자용 연동 가이드를 `documents/03_기능_명세/` 하위에 작성 완료했습니다.
...
*   **예매 상태 관리 및 이력 필터링 개선**:
    ...
    *   **API 문서 명확화**: `POST /api/bookings/released` 및 `DELETE /api/bookings/{id}` API가 결제 전후 상태에 따라 다르게 동작(단순 해제 vs 실제 취소)함을 Swagger 문서에 상세히 기술했습니다.
...
*   **공지사항(Notice) 및 관리자 공지사항 API 고도화 [CCS-55]:**
    ...
    *   **단일 활성 규칙 강제**: 관리자가 특정 공지사항을 활성화(`isActive=true`)할 때만 시스템이 개입하여 동일 카테고리의 다른 공지사항들을 자동으로 비활성화 처리합니다.
    *   **API 엔드포인트 최적화**: 
        *   사용자 API를 카테고리별 그룹화 조회(`GET /api/notices/grouped`)로 단일화했습니다.
        *   `/api/notices/grouped` 호출 시 각 카테고리 내 활성화된(`isActive=true`) 공지사항들을 반환합니다.
    *   **검증 강화**: `NoticeServiceTest`를 통해 신규 활성화 프로세스와 단일 활성 규칙에 대한 단위 테스트를 완료했습니다.
## 2026년 2월 7일 (토)

*   **Swagger API 그룹화 기능 추가 [CCS-160]:**
    *   **가독성 개선**: API 목록이 방대해짐에 따라 Swagger UI 상단 드롭다운을 통해 User API와 Admin API를 분리해서 볼 수 있도록 그룹화 기능을 도입했습니다.
    *   **그룹 분류**: `/api/admin/**` 경로는 Admin API로, 그 외 경로는 User API로 자동 분류됩니다.

*   **마이페이지 및 관리자용 예매 내역 조회 기능 구현 [CCS-158]:**
    *   **사용자용 내역/상세**: 사용자가 자신의 예매 목록(`GET /api/bookings/me`)과 공연/결제 정보가 포함된 상세 데이터(`GET /api/bookings/{bookingId}`)를 조회할 수 있는 API를 구현했습니다.
    *   **관리자용 현황/상세**: 특정 회차의 전체 예매 목록(`GET /api/admin/bookings/schedules/{id}`) 및 특정 예매 건의 상세 내역 조회 API를 구축했습니다.
    *   **데이터 통합 제공**: 예매, 공연, 좌석 상세, 결제 승인 정보를 한눈에 확인할 수 있는 통합 응답 구조를 적용했습니다.
    *   **가이드 신설**: 상세 명세를 담은 `예매_내역_조회_가이드.md`를 새로 작성했습니다.

*   **예매 취소 및 환불 통합 프로세스 구축 [CCS-156]:**
    *   **단일 진입점 통합**: 중복된 결제 취소 API를 제거하고 `DELETE /api/bookings/{id}` API로 취소 프로세스를 단일화했습니다.
    *   **자동 환불 및 쿠폰 복구**: 확정된 예매 취소 시 결제 시스템과 연동하여 PG 환불 및 사용된 쿠폰을 자동으로 복구하도록 로직을 강화했습니다.
    *   **관리자 강제 취소**: `DELETE /api/admin/bookings/{id}`를 통해 관리자가 소유권 검증 없이 예매를 취소할 수 있는 기능을 추가하고 Audit Log를 적용했습니다.
    *   **문서 최신화**: `예매_취소_및_환불_워크플로우.md`에 통합 프로세스 및 FE 테스트 가이드를 반영했습니다.
...
*   **홈 화면 카테고리 시스템 구현 [CCS-145]:**
    ...
    *   **문서화:** 설계서(`홈_카테고리_시스템_설계.md`)와 FE 개발자용 가이드(`홈_화면_API_가이드.md`)를 작성했습니다.
...
*   **사용자/관리자용 공연 조회 API 분리 및 노출 필터링 적용 [CCS-144]:**
    ...
    *   **관리자용 API (/api/admin/performances):** 숨김 처리된 공연을 포함한 전체 목록을 조회할 수 있는 전용 엔드포인트를 추가했습니다.
    *   **도메인 로직 분리:** `PerformanceService`에 `getAllPerformances()`와 `getVisiblePerformances()`를 명확히 분리하여 의도를 명확히 했습니다.
    *   **코드 품질 개선:** `PaymentItem` 엔티티 내 불필요한 non-null assertion(!!)을 제거하여 빌드 경고를 해결했습니다.

*   **통계 재계산 오류 긴급 수정 (Hotfix):**
    *   **LazyInitializationException 해결:** 주간 통계 재계산 스케줄러(`StatsRecalculationScheduler`)에 `@Transactional`을 적용하여 지연 로딩 컬렉션 접근 시 발생하던 오류를 해결했습니다.
    *   **공연별 통계 재집계 로직 오류 수정:** 공연별 통계(`PerformanceSalesStats`) 삭제 시 잘못된 조건(ID=0)으로 조회하던 버그를 수정하여, 날짜 기준(`findAllByDate`)으로 정상 삭제 후 재집계되도록 개선했습니다.

*   **결제 검증 시스템 에러 메시지 상세화 및 디버깅 편의성 개선 [CCS-140]:**
    *   **원가 검증 상세 내역 노출:** 결제 요청 원가와 서버 계산 합계가 불일치할 경우, 단순히 합계만 비교하는 것이 아니라 예매된 모든 좌석의 상세 정보(ID, 구역, 행/열, 가격)를 에러 메시지에 포함하여 원인 파악을 즉시 할 수 있도록 개선했습니다.
    *   **쿠폰 적용 좌석 검증 강화:** 쿠폰이 적용된 좌석 ID가 해당 예매 내역에 존재하지 않을 경우, 요청된 ID와 함께 현재 예매된 유효한 좌석 ID 목록을 메시지에 출력하여 클라이언트의 요청 오류를 명확히 안내합니다.
    *   **로깅 컨텍스트 추가:** 원가 불일치 오류 시 예매 ID(`bookingId`)를 메시지에 포함하여 서버 로그 추적 시의 편의성을 높였습니다.

## 2026년 1월 22일 (목)

*   **결제 금액 검증 강화 및 에러 메시지 개선 [CCS-140]:**
    *   **원가 사전 검증 추가:** 결제 생성 시 클라이언트가 전송한 `originalPrice`와 서버의 실제 좌석 가격 합계를 비교하여 불일치 시 조기에 명확한 에러 메시지를 반환하도록 개선했습니다. (`PaymentService.createPayment`)
    *   **금액 검증 에러 메시지 가독성 개선:** `Payment.validateAmount()`의 에러 메시지를 계산 과정이 한눈에 보이도록 구조화하여 디버깅 편의성을 향상시켰습니다. (원가, 수수료, 할인, 최종가 항목별 표시 및 차이 금액 명시)
    *   **테스트 강화:** 원가 불일치 검증 시나리오에 대한 단위 테스트(`createPaymentThrowsIfOriginalPriceMismatch`)를 추가했습니다.
...
*   **결제 시스템 안정성 강화 (Upsert 로직 도입) [CCS-138]:**
    *   **중복 결제 요청(Retry) 지원:** 결제 실패 또는 취소 후 재시도 시 발생하는 `DataIntegrityViolationException` 해결을 위해 `PaymentService.createPayment`에 Upsert(조회 후 갱신) 로직을 적용했습니다.
    *   **Payment 엔티티 개선:** 기존 결제 레코드를 재사용할 수 있도록 상태 및 데이터를 초기화하는 `update()` 메서드를 추가했습니다.
    *   **쿠폰 복구 로직 통합:** 재결제 시 기존에 적용된 쿠폰이 있다면 자동으로 복구(`restoreCoupons`) 후 재적용되도록 프로세스를 개선했습니다.
    *   **테스트 강화:** 결제 재시도 시나리오에 대한 서비스 단위 테스트(`createPaymentReusesExistingPayment`)를 추가하여 안정성을 검증했습니다.

## 2026년 1월 11일 (일)

*   **예매 시스템 점유 시간 단축 및 정책 일원화:**
    *   **점유 시간 변경:** 예매 및 좌석 점유 유효 시간을 기존 5분에서 **2분**으로 단축하여 회전율을 개선했습니다.
    *   **정책 상수 통일:** `BookingPolicy` 객체를 도입하여 `Booking` 엔티티와 `ScheduleSeatStatus`의 점유 시간을 하나의 상수로 관리, 정책 불일치를 방지했습니다.

*   **쿠폰 시스템 개선 및 정렬 기능 추가:**
    *   **정렬 순서(Sort Order) 도입:** `Coupon` 엔티티 및 관련 DTO에 `sortOrder` 필드를 추가하여 관리자가 쿠폰 표시 순서를 제어할 수 있도록 개선했습니다.
    *   **'일반' 쿠폰(할인 없음) 지원:** 쿠폰 선택 화면에서 미적용 상태를 명시적으로 선택할 수 있도록, 쿠폰 목록 조회 API(`/api/coupons/me`) 최상단에 '일반(할인율 0%)' 가상 쿠폰을 추가했습니다.
    *   **예외 처리:** '일반' 쿠폰(ID: 0) 선택 시 할인 계산 및 사용 이력 저장 로직에서 예외 처리하여 정상 동작하도록 구현했습니다.

*   **결제 API 문서화(Swagger) 보완:**
    *   **Schema 주석 전수 추가:** `PaymentDto.kt` 내의 모든 요청/응답 DTO(`PaymentCreateRequest`, `PaymentCancelRequest`, `PaymentResponse` 등)에 `@Schema` 어노테이션을 상세히 추가하여 API 문서의 가독성을 높였습니다.
